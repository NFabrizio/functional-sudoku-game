...
const addAllClickEvents = () => {
  return _.each(document.getElementsByClassName('js-sudoku-cell'), addClickEvent);
};
...
export const renderTable = table => {
  ...
  document.getElementsByClassName('js-table')[0].innerHTML = tableTemplate({ table, isValid });
...
export const renderTimer = (t=0) => {
  document.getElementsByClassName('js-timer')[0].innerHTML = _.template('<%= t %> seconds')({ t });
  // `${t} seconds`;
